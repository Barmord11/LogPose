...
describe('fetchWatchEpisodes', () => {
  it('returns the episode list with outbound AnimeKai urls, by AniList id', async () => {
  ...
  it('returns an empty array when the provider has no episodes', async () => {
    mockAnilistInstance.fetchAnimeInfo.mockResolvedValue({ id: 1 })
  ...
  it('throws ConsumetLookupError when the provider throws (e.g. AnimeKai is down)', async () => {
    mockAnilistInstance.fetchAnimeInfo.mockRejectedValue(new Error('Cloudflare 522'))
